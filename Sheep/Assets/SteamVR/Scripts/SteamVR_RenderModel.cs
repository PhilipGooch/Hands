        ...
        IEnumerator SetModelAsync(string newRenderModelName)
        {
            ...
            using (RenderModelInterfaceHolder holder = new RenderModelInterfaceHolder())
            {
                ...
                while (true)
                {
                    ...
                    {
                        if (!SteamVR.enabled || SteamVR.instance == null || OpenVR.System == null)
                        {
                            // Don't do anything if SteamVR is offline!
                            // Hopefully fixed the random crashes when entering/exiting play mode
                            yield break;
                        }
