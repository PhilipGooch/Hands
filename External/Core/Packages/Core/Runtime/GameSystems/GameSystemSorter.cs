...
namespace NBG.Core.GameSystems
{
    public class GameSystemSorter
    {
        ...
        /// <summary>
        /// This heap class orders ambiguous systems based on type name.
        /// The easiest way to change the ordering would be to change CompareTo method of TypeHeapElement.
        /// </summary>
        private class Heap
        {
        ...
        public struct TypeHeapElement : IComparable<TypeHeapElement>
        {
            ...
            {
                unsortedIndex = index;
                typeName = GameSystemSorter.SysName(t);
            }

            /// <summary>
            /// The crux of resolving ambiguous system order.
            /// </summary>
        ...
        internal static void Sort(SystemElement[] elements)
        {
            ...
            while (!readySystems.Empty)
            {
                ...
                foreach (var beforeType in elem.updateBefore)
                {
                    ...
                    if (beforeIndex < 0) throw new Exception("Bug in GameSystemSorter.Sort(), beforeIndex < 0");
                    if (elements[beforeIndex].nAfter <= 0)
                        throw new Exception("Bug in GameSystemSorter.Sort(), nAfter <= 0");
        ...
        internal static string SysName(Type stype)
        {
                ...
                foreach (var attr in before)
                {
                    ...
                    if (depIndex < 0)
                    {
                        Debug.LogWarning(
                            ...
                            + $"This attribute can only order systems that are members of the same {nameof(GameSystemGroup)} instance.\n"
        ...
        {
            if (!typeof(GameSystemBase).IsAssignableFrom(dep.SystemType))
            {
                Debug.LogWarning(
                    $"Ignoring invalid [UpdateBefore] attribute on {SysName(systemType)} because {SysName(dep.SystemType)} is not a subclass of {nameof(GameSystemBase)}.\n"
                    + $"Set the target parameter of [UpdateBefore] to a system class in the same {nameof(GameSystemGroup)} as {SysName(systemType)}.");
                return true;
            }
            ...
            if (dep.SystemType == systemType)
            {
                Debug.LogWarning(
                    ...
                    + $"Set the target parameter of [UpdateBefore] to a different system class in the same {nameof(GameSystemGroup)} as {SysName(systemType)}.");
                return true;
            }

            int systemBucket = GameSystemGroup.ComputeSystemOrdering(systemType, parentType);
            int depBucket = GameSystemGroup.ComputeSystemOrdering(dep.SystemType, parentType);
        ...
        private static bool CheckAfterConstraints(Type parentType, UpdateAfterAttribute dep, Type systemType)
        {
            ...
            {
                Debug.LogWarning(
                    $"Ignoring invalid [UpdateAfter] attribute on {SysName(systemType)} because {SysName(dep.SystemType)} is not a subclass of {nameof(GameSystemBase)}.\n"
                    + $"Set the target parameter of [UpdateAfter] to a system class in the same {nameof(GameSystemGroup)} as {SysName(systemType)}.");
                return true;
            }
            ...
            if (dep.SystemType == systemType)
            {
                Debug.LogWarning(
                    ...
                    + $"Set the target parameter of [UpdateAfter] to a different system class in the same {nameof(GameSystemGroup)} as {SysName(systemType)}.");
                return true;
            }
