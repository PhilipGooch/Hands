...
namespace NBG.Core
{
    ...
    {
        public static Vector3 GetClosestMajorAxis(Quaternion rot, Vector3 target, Vector3 axisMask)
        {
            ...
            axis = rot * Vector3.right;
            dot = Vector3.Dot(target, axis) * axisMask.x;
            ...
            axis = rot * Vector3.up;
            dot = Vector3.Dot(target, axis) * axisMask.y;
            if (Mathf.Abs(dot) > bestDot)
            {
                var sign = Mathf.Sign(dot);
                bestAxis = sign * axis;
            ...
            axis = rot * Vector3.forward;
            dot = Vector3.Dot(target, axis) * axisMask.z;
            if (Mathf.Abs(dot) > bestDot)
            {
                var sign = Mathf.Sign(dot);
                bestAxis = sign * axis;
        ...
        public static bool LineLineIntersection(out float tLine1, out float tLine2, Vector3 linePoint1, Vector3 lineVec1, Vector3 linePoint2, Vector3 lineVec2)
        {
            ...
            //Note: sqrMagnitude does x*x+y*y+z*z on the input vector.
            tLine1 = Vector3.Dot(crossVec3and2, crossVec1and2) / crossVec1and2.sqrMagnitude;
            tLine2 = Vector3.Dot(crossVec3and1, crossVec1and2) / crossVec1and2.sqrMagnitude;
            return true;
        ...
        // Same as closes point 2 lines, but second line is a segment - clamped within the line
        ...
        {
            var segmentVec = segmentPoint2 - segmentPoint1;
            ...
            float b = Vector3.Dot(lineVec, segmentVec);
            float e = Vector3.Dot(segmentVec, segmentVec);
            ...
            if (d != 0.0f)
            {
                ...
                Vector3 r = linePoint - segmentPoint1;
                float c = Vector3.Dot(lineVec, r);
                float f = Vector3.Dot(segmentVec, r);
                ...
                closestPointLine = linePoint + lineVec * s;
                closestPointSegment = segmentPoint1 + segmentVec * segmentT;
        ...
        // same as ProjectPointOnLine, but returns t
        public static Vector3 ProjectPointOnLine(out float t, Vector3 linePoint, Vector3 lineVec, Vector3 point)
        {
            ...
            t = Vector3.Dot(linePointToPoint, lineVec);
        ...
        {
            // FIX: Unity Quaterion.ToAngleAxis fails for q = new Quaternion(0,0,0,-1);
            if (q.w < -0f) q = new Quaternion(-q.x, -q.y, -q.z, -q.w);

            float angle;
            Vector3 axis;
            q.ToAngleAxis(out angle, out axis);
            angle = Math2d.NormalizeAngleDeg(angle);
            angle *= Mathf.Deg2Rad;
            var errorPos = angle * axis;
            return errorPos;
        }

        public static Vector3 QToAngleAxis(this Quaternion q)
        {
            return QuaternionToAngleAxis(q);
        }

        public static Quaternion AngleAxisToQuaternion(this Vector3 rot)
        {
            return Quaternion.AngleAxis(rot.magnitude * Mathf.Rad2Deg, rot.normalized);
        }
        // http://www.euclideanspace.com/maths/geometry/rotations/for/decomposition/forum.htm
        public static void QuaternionToSwingTwist(Quaternion q, Vector3 axis, out Quaternion twist, out Quaternion swing)
        {
            var rotatedAxis = q * axis;
            swing = Quaternion.FromToRotation(axis, rotatedAxis);
            twist = Quaternion.Inverse(swing) * q;


            // var rotation_axis = new Vector3(q.x, q.y, q.z);

            // // return projection v1 on to v2 (parallel component)
            // // here can be optimized if default_dir is unit
            // var proj = Vector3.Project( rotation_axis, axis );
            ...
            // #ifdef _DEBUG
            // var  composite = swing * twist;
            // composite -= orientation;
            // ASSERT( composite.magnitude() < 0.00001f );
            // #endif //_DEBUG 
        }

        public static float GetTwist(Quaternion q, Vector3 axis)
        {
            Math3d.QuaternionToSwingTwist(q, axis, out var twistQ, out var swingQ);

            // calculate twist
            twistQ.ToAngleAxis(out var twist, out var a);
            if (Vector3.Dot(axis, a) > 0)
                return Math2d.NormalizeAngleDeg(twist);
            else
                return -Math2d.NormalizeAngleDeg(twist);
        }

        public static Quaternion GetPlaneRotation(Vector3 p1, Vector3 p2, Vector3 p3)
        {
